    ...
    public static function atPath(string $path): self
    {
        ...
        if (is_int($schema) && $schema >= 1 && $schema < self::SCHEMA) {
            throw new FailureException(sprintf(
                'The vet file [%s] declares schema [%d], which %s. Schema [%d] records the version and the full tree hash of each package that you trust. Delete the file and run [vet trust --all] again.',
                $path,
                $schema,
