...
it('reads a file that an autoload rule points at as runtime source', function (): void {
    expect(new ClassifyPath(['src'], false)->handle('src/Widget.php', classifyFile('')))->toBe(BucketType::RuntimeSource);
});

it('reads a Dockerfile as runtime source', function (): void {
    $classifier = new ClassifyPath(['src'], false);

    expect($classifier->handle('Dockerfile', classifyFile('')))->toBe(BucketType::RuntimeSource)
        ->and($classifier->handle('runtimes/8.4/Dockerfile', classifyFile('')))->toBe(BucketType::RuntimeSource)
        ->and($classifier->handle('runtimes/8.4/php.dockerfile', classifyFile('')))->toBe(BucketType::RuntimeSource)
        ->and($classifier->handle('docker-compose.yml', classifyFile('')))->toBe(BucketType::RuntimeSource);
});

it('reads a shell script as runtime source', function (): void {
    expect(new ClassifyPath(['src'], false)->handle('runtimes/8.4/start-container.sh', classifyFile('')))
...
it('reads a file that starts with a shebang as runtime source', function (): void {
    ...
    expect(new ClassifyPath(['src'], false)->handle('runtimes/8.4/start-container', $file))
        ->toBe(BucketType::RuntimeSource);
});
...
it('reads a document as inert', function (): void {
    ...
    expect($classifier->handle('README.md', classifyFile("# Widget\n")))->toBe(BucketType::Inert)
        ->and($classifier->handle('tests/WidgetTest.php', classifyFile('')))->toBe(BucketType::Inert);
});

it('reads composer.json as the install manifest', function (): void {
    expect(new ClassifyPath(['src'], false)->handle('composer.json', classifyFile('')))->toBe(BucketType::InstallManifest);
});

it('reads a compiled binary as opaque, even inside an autoload root', function (): void {
    $classifier = new ClassifyPath(['src'], false);

    expect($classifier->handle('src/ext/widget.so', classifyFile('')))->toBe(BucketType::Opaque)
        ->and($classifier->handle('src/Widget.PHAR', classifyFile('')))->toBe(BucketType::Opaque);
});

it('reads a file that holds a null byte as opaque', function (): void {
    expect(new ClassifyPath(['src'], false)->handle('src/Widget.php', classifyFile("<?php\n\0\0payload")))
...
it('reads a large script on one line as opaque', function (): void {
    expect(new ClassifyPath(['src'], false)->handle('dist/widget.min.js', classifyFile(str_repeat('var a=1;', 3000))))
        ->toBe(BucketType::Opaque);
});
...
it('reads each php file as runtime source when an autoload rule maps the package root', function (): void {
    ...
    expect($classifier->handle('Widget.php', classifyFile('')))->toBe(BucketType::RuntimeSource)
        ->and($classifier->handle('tests/WidgetTest.php', classifyFile('')))->toBe(BucketType::RuntimeSource)
        ->and($classifier->handle('README.md', classifyFile('')))->toBe(BucketType::Inert);
});
