...
it('reads the operations of a real dry run', function (): void {
    $plan = ComposerPlan::parse((string) file_get_contents(dirname(__DIR__, 2).'/Fixtures/composer-update-dry-run.txt'));
...
it('names no install for a dev package when the tree holds no dev package', function (): void {
...
it('names an install for a dev package when the tree holds dev packages', function (): void {
    $fixture = Fixture::open('no-dev-install');
    ...
    expect($plan->of('acme/lint')?->change)->toBe(ComposerChangeType::Install);
});
