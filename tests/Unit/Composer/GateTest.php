...
it('restores composer.lock to the content that it held before the update', function (): void {
    $gate = gateProject(trustFile: true, binary: true);

    mkdir($gate->rootPath.'/vendor/composer', 0o777, true);
    file_put_contents($gate->lockPath(), '{"packages":["before"]}');

    $gate->backupLock();

    file_put_contents($gate->lockPath(), '{"packages":["after"]}');

    try {
        expect($gate->restoreLock())->toBeTrue()
            ->and(file_get_contents($gate->lockPath()))->toBe('{"packages":["before"]}');

        $gate->deleteLockBackup();

        expect(is_file($gate->lockBackupPath()))->toBeFalse()
            ->and($gate->restoreLock())->toBeFalse();
    } finally {
        @unlink($gate->lockPath());
        @unlink($gate->lockBackupPath());
        @rmdir($gate->rootPath.'/vendor/composer');
    }
});

it('backs up no composer.lock when the project holds none', function (): void {
    $gate = gateProject(trustFile: true, binary: true);
    ...
    try {
        ...
        expect(is_file($gate->lockBackupPath()))->toBeFalse()
            ->and($gate->restoreLock())->toBeFalse()
            ->and(is_file($gate->lockPath()))->toBeFalse();
    } finally {
        @rmdir($gate->rootPath.'/vendor/composer');
    }
});
